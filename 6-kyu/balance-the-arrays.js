...
function balance(arr1, arr2) {
    ...
    const sortedArr1ValuesFrequencies = [...arr1ValuesFrequencies.values()].sort(function sortAsc(a, b) {
        return a - b;
    });
    // sort arr2 freq counts
    const sortedArr2ValuesFrequencies = [...arr2ValuesFrequencies.values()].sort(function sortAsc(a, b) {
        return a - b;
    });
    // iterate larger arr, check for same values on each iteration
    const largestArr = sortedArr1ValuesFrequencies.length >= sortedArr2ValuesFrequencies.length ? sortedArr1ValuesFrequencies : sortedArr2ValuesFrequencies;
