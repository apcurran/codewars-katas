...
function sortTransform(nums) {
    ...
    const convertedChars = nums
        .map(function convertNumsToASCII(num) {
...
function getCharsChunk(nums) {
    return (
        String.fromCharCode(nums[0]) +
        String.fromCharCode(nums[1]) +
        String.fromCharCode(nums.at(-2)) +
        String.fromCharCode(nums.at(-1))
    );
}
